  ...
  async create(createMedicationDto: CreateMyPatientsDto) {
    try {
      const patientFetched = await this.userRepository.findOneBy({
        id: createMedicationDto.patient,
      });

      const doctorFetched = await this.userRepository.findOneBy({
        id: createMedicationDto.doctor,
      });
      ...
      return {
        statusCode: 201,
        method: 'POST',
        message: 'my patients created sucessfully',
        data: { ...Datasaved },
      ...
      console.log(
        `Failed to create new my patients | Error Message: ${error.message}`,
      );
